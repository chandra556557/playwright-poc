...
class TextFuzzyMatchStrategy extends BaseHealingStrategy {
  constructor(page) {
  ...
  async generateCandidates(context) {
    ...
    if (!textContent || textContent.length  w.length > 2);
  ...
  async findSimilarTextElements(context) {
    const candidates = [];
    const { textContent, tagName } = context;
  ...
  async generateSelectorForElement(element) {
    try {
  ...
  isTextSimilar(text1, text2, threshold = 0.65) {
    ...
    const levSim = maxLength > 0 ? 1 - (distance / maxLength) ;
  ...
  async calculateFeatures(
    ...
    features.levenshtein = maxLen > 0 ? 1 - (levenshtein.get(norm1, norm2) / maxLen) ;

    // Tag match
    features.tagMatch = candidate.tagName === original.tagName ? 1 ;
    ...
    // Visibility match
    features.visibility = candidate.isVisible === original.isVisible ? 1 ;
    ...
    features.lengthSimilarity = originalLen > 0 ? 
      1 - Math.abs(originalLen - candidateLen) / Math.max(originalLen, candidateLen) ;

    return features;
  }
}

module.exports = { TextFuzzyMatchStrategy };