...
// Anchor proximity strategy: use nearby stable anchors/labels to locate element when text changes
export class AnchorProximityStrategy extends BaseHealingStrategy {
  constructor(page: any) {
    super(page, 'anchor-proximity');
  }

  private async findNearAnchors(context: ElementContext): Promise<string[]> {
  ...
  private async generateSelectorFor(element: any): Promise<string | null> {
    try {
      const [tagName, attrs, txt] = await Promise.all([
        element.evaluate((el: Element) => el.tagName.toLowerCase()),
        element.evaluate((el: Element) => {
          const o: Record<string,string> = {}; for (const a of Array.from(el.attributes)) o[a.name]=a.value; return o;
        }),
